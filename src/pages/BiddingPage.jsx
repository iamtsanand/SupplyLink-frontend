...
import { Users, BarChart, Handshake, TrendingDown, ShieldAlert, Tag, CheckCircle, XCircle, Clock } from 'lucide-react';
...
// A new, reusable Toast component for notifications
const Toast = ({ message, type, onClose }) => {
    ...
    return (
        <div className={`fixed top-5 right-5 flex items-center px-4 py-3 rounded-lg shadow-lg text-white ${bgColor} animate-fade-in-down`}>
...
const BiddingPage = () => {
    ...
    // State to track time until the next window
    const [nextWindow, setNextWindow] = useState(getTimeToNextWindow());
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setNextWindow(getTimeToNextWindow());
        }, 60000); // 60 seconds
    ...
    const handleBidSubmit = async (e, item, currentLowestBid) => {
        e.preventDefault();
        if (!userId || !user) {
            showToast("You must be signed in to place a bid.", 'error');
            return;
        }

        const priceNum = Number(bidPrice);
        if (currentLowestBid && priceNum >= currentLowestBid) {
            showToast(`Your bid must be lower than the current lowest bid of ₹${currentLowestBid}.`, 'error');
            return;
        }
        ...
        try {
            ...
            showToast(`Bid Placed/Updated Successfully!`, 'success');
        ...
        return (
            <div className="relative">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    {/* Conditional rendering based on bidding window */}
                    {!biddingActive ? (
                        <div className="text-center py-10 px-6 bg-blue-50 border border-blue-200 rounded-lg shadow-md">
                            <Clock className="mx-auto h-12 w-12 text-blue-500" />
                            <h3 className="mt-4 text-xl font-semibold text-blue-800">Bidding Window is Closed</h3>
                            <p className="text-blue-600 mt-2">The next bidding session starts in approximately <span className="font-bold">{nextWindow}</span>.</p>
                            <p className="text-sm text-gray-500 mt-1">Windows are 8-9 AM and 8-9 PM IST.</p>
                        </div>
                    ) : (
                        <div className="space-y-6">
                            {Object.keys(aggregatedDemands).length > 0 ? Object.entries(aggregatedDemands).map(([item, data]) => (
                                ...
                                <div className="text-center py-10 px-6 bg-white rounded-lg shadow-md">
                                    ...
                                    <p className="text-gray-500 mt-2">There are no open requirements from vendors in your state during this bidding window.</p>
                                </div>
                            )}
                        </div>
                    )}
